...
const ICONS = {
  confirm: <Info size={22} style={{color:'var(--teal)'}} />,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"
      ...
      <div className="mystical-card corner-decor dialog-enter w-full max-w-sm p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">{ICONS[type]}<h3 className="font-bold text-base gold-glow font-cinzel" style={{color:'var(--gold)'}}>{title}</h3></div>
          {onCancel && <button onClick={onCancel} className="transition ml-2" style={{color:'var(--text-muted)'}}><X size={18}/></button>}
        </div>
        <hr className="divider-gold mb-4"/>
        <p className="text-sm leading-relaxed mb-6 whitespace-pre-line font-noto" style={{color:'var(--text-secondary)'}}>{message}</p>
        <div className="flex gap-3 justify-end">
          {onCancel && (
            <button onClick={onCancel} className="px-4 py-2.5 rounded-lg text-sm transition font-noto"
              style={{ border:'1px solid var(--border-card)', background:'var(--bg-option)', color:'var(--text-muted)' }}>
              {cancelLabel}
            </button>
          )}
          <button onClick={onConfirm} className={`px-5 py-2.5 rounded-lg text-sm font-bold transition ${btnStyle[type]}`}>
            {confirmLabel}
          </button>
        </div>
      </div>
