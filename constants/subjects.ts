export interface SubjectConfig {
  name: string; icon: string; color: string; levels: string[];
}
export const SUBJECTS_CONFIG: SubjectConfig[] = [
  { name:'JLPT',    icon:'🇯🇵', color:'text-red-400 border-red-700/50 bg-red-900/30',    levels:['N1','N2','N3','N4','N5'] },
  { name:'TOEIC',   icon:'🇺🇸', color:'text-blue-400 border-blue-700/50 bg-blue-900/30', levels:['900+','750+','600+','450+','300+'] },
  { name:'English', icon:'🌐', color:'text-sky-400 border-sky-700/50 bg-sky-900/30',     levels:['C2','C1','B2','B1','A2','A1'] },
  { name:'Math',    icon:'📐', color:'text-green-400 border-green-700/50 bg-green-900/30',levels:['Advanced','Intermediate','Basic'] },
  { name:'Khác',    icon:'📚', color:'text-purple-400 border-purple-700/50 bg-purple-900/30',levels:['Advanced','Intermediate','Basic'] },
];
export const ALL_SUBJECT_NAMES = ['Tất cả', ...SUBJECTS_CONFIG.map(s => s.name)];
export function getSubjectConfig(name: string) { return SUBJECTS_CONFIG.find(s => s.name === name); }
